<?php
    ...
    if(@$_GET["method"] == "login"){
        ...
        $mid = $_SESSION['mid'];
        $sql = "SELECT * FROM MEMBER WHERE EMAIL = '$email' AND MID != '$mid'";
        $stid = oci_parse($conn, "$sql") or die($e = oci_error().trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR));
        ...
        if(oci_execute($stid)){
            ...
            if($row == ''){
                $sql2 = "UPDATE MEMBER 
                        SET EMAIL = '$email', PASS = '$password', M_NAME = '$m_name', ADDRESS = '$address', BIRTHDAY = to_date('". $birthday."','yyyy-mm-dd'), PHONE = '$phone'
                        WHERE MID = '$mid'";
                $stid2 = oci_parse($conn, "$sql2") or die($e = oci_error().trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR));
                if(oci_execute($stid2)){
                    $status = 1;
